...
def optimality_criteria(rho, rho_min, rho_max, V, dCdrho, dVdrho, move=0.05):
    ...
    while ub - lb > 1e-4:
        mid = (lb + ub) / 2.0
        rho_new = np.maximum.reduce(
            [
                np.minimum.reduce(
                    [
                        rho * (-dCdrho / (dVdrho + 1e-12) / mid) ** 0.5,
                        rho + move,
                        rho_max,
                    ]
                ),
...
def mma_optimizer(
    ...
    Args:
        ...
        df0dx           : The derivatives of the objective function, df_0(x)dx_j, calculated at xval.
        fval            : The values of the constraint functions, f_i(x), calculated at xval.
        dfdx            : (m, n) array, derivatives of the constraint functions, df_i(x)dx_j, at xval.
    ...
    ul1 = upp - low
    p0, q0 = (p0 + feps / ul1) * (upp - xval) ** 2, (q0 + feps / ul1) * (
        xval - low
    ) ** 2
    ...
    x_new = solve_subproblem(
        m, epsimin, low, upp, alpha, beta, p0, q0, P_mat, Q_mat, a0, a, b, c, d
    )
...
def solve_subproblem(
    ...
    while eps > epsimin:  # Primal-dual interior point method
        ...
        newton_iter = 0
        while (
            res_max > 0.9 * eps and newton_iter < 100
        ):  # Newton's method for the search direction
            newton_iter += 1
            if newton_iter == 100 and comm.rank == 0:
                print(
                    "Maximum inner iterations reached in the MMA optimizer", flush=True
                )
            ...
            diag_x = (
                2 * (p_lambda / (upp - x) ** 3 + q_lambda / (x - low) ** 3)
                + xi / (x - alpha)
                + eta / (beta - x)
            )
            ...
            if comm.rank == 0:
                ...
                diag_y = d + mu / y
                A_mat = (
                    sparse.spdiags(s / _lambda + 1.0 / diag_y, 0, m, m)
                    + G_mat_global * (1.0 / diag_x_global) @ G_mat_global.T
                )
                lhs = np.vstack(
                    [np.hstack([A_mat, a.reshape(-1, 1)]), np.hstack([a, -zeta / z])]
                )
            ...
            if comm.rank == 0:
                ...
                b_lambda = (
                    delta_lambda
                    + delta_y / diag_y
                    - G_mat_global @ (delta_x_global / diag_x_global)
                )
            ...
            step_inv_alpha = np.max(
                -1.01 * incr_x / (x - alpha), initial=0
            )  # Ensure x-alpha >= 0.01/1.01*(x_old-alpha)
            step_inv_beta = np.max(
                1.01 * incr_x / (beta - x), initial=0
            )  # Ensure beta-x >= 0.01/1.01*(beta-x_old)
            step_inv_alpha = comm.allreduce(step_inv_alpha, op=MPI.MAX)
            step_inv_beta = comm.allreduce(step_inv_beta, op=MPI.MAX)
            if comm.rank == 0:
                step_inv = max(
                    -1.01 * incr / values
                )  # Ensure y >= 0.01/1.01*y_old, etc.
                step_inv = max(
                    step_inv_alpha, step_inv_beta, step_inv, 1.0
                )  # Ensure step <= 1
                step = 1.0 / step_inv
            else:
                step = None
            ...
            while (
                res_norm_new > res_norm and search_iter < 50
