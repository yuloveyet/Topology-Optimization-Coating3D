...
from fenitop.utility import Communicator, Plotter, save_xdmf, get_date_str
...
def topopt(fem, opt):
    ...
    loop, opt_iter, beta, change = 0, 0, 1, 2 * opt["opt_tol"]
    while opt_iter < opt["max_iter"] and change > opt["opt_tol"]:
        ...
        if opt["opt_compliance"]:
            ...
            g_vec = np.array(
                [V_value - opt["vol_frac"], C_value - opt["compliance_bound"]]
            )
        ...
        if comm.rank == 0:
            print(
                f"It.: {loop} C.: {C_value:.3f} Vf.: {V_value:.3f} "
                f"U.: {U_value:.3f} beta: {beta} "
                f"change: {change:.3f} t.: {opt_time: .3f}(s)",
                flush=True,
            )
        ...
        if loop % plot_freq == 0 or change <= opt["opt_tol"] or opt_iter >= opt["max_iter"]:
            ...
            if comm.rank == 0:
                plotter.plot(values, loop, path=filepath)

    save_xdmf(fem["mesh"], rho_phys_field, path=filepath)
