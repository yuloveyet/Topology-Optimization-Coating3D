...
import pyvista, datetime

def get_date_str(comm=None):
    """Get current date string (UTC+8) for file path, synchronized across MPI processes."""
    if comm is None or comm.rank == 0:
        # Use UTC+8 (Beijing Time) explicitly
    ...
    if comm is not None:
        date_str = comm.bcast(date_str, root=0)
...
class Communicator:
    ...
    def __init__(self, func_space, mesh_serial, size=1):
        ...
        local_nodal_range = np.asarray(
            idx_map.local_range, dtype=np.int32
        )  # [start, end]
        ...
        if self.comm.rank == 0:
            ...
            def node2dof(nodes, num_nodal_dofs):
                return (
                    np.tile(nodes, (num_nodal_dofs, 1)) * num_nodal_dofs
                    + np.arange(num_nodal_dofs).reshape(-1, 1)
                ).ravel("F")
            ...
            self.local_to_global_dofs = (
                np.tile(self.local_to_global_dofs.reshape(-1, 1), (1, size)) * size
                + np.arange(size)
            ).ravel()
        else:
