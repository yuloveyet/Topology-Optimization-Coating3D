...
def main():
    ...
    dx_m, dy_m, dz_m = length / mesh_res_phys[0], height / mesh_res_phys[1], width / mesh_res_phys[2]
    ...
    fem = {
        ...
        & (x[0] >= -1e-6) & (x[0] <= length + 1e-6) & (x[2] >= -1e-6) & (x[2] <= width + 1e-6)
        & (np.less(x[0], 1.5 + dx_m/2 + 1e-6) | np.greater(x[0], length - 1.5 - dx_m/2 - 1e-6)),
        "traction_bcs": [
            [
                (0.0, 0.0, -2.0),
    ...
    opt = {
        ...
        "void_zone": lambda x: (
            (x[0] < -1e-6) | (x[0] > length + 1e-6) | 
            (x[1] < -1e-6) | (x[1] > height + 1e-6) | 
            (x[2] < -1e-6) | (x[2] > width + 1e-6)
        ),
        "penalty": 3.0,
        "epsilon": 1e-6,
    ...
    # Initialize rho_field for a non-trivial starting point
    ...
    def forward_pass(current_beta):
        ...
        # In current model, rho_shell_func is NOT used in the physics, rho_shell_expr is used directly.
        # But we still update it for visualization consistency.
        rho_shell_func_new = project_expression(rho_shell_expr, V=S, mesh=mesh, quadrature_degree=fem["quadrature_degree"])
    ...
    # --- Cleanup ---
    dC_drnf_vec.destroy(); dV_drnf_vec.destroy()
    dC_drb_direct_vec.destroy(); dV_drb_direct_vec.destroy()
    ...
    dist_to_axis = np.sqrt((centers[:, 0] - length / 2.0)**2 + (centers[:, 2] - width / 2.0)**2)

    # Local Candidates: 1 Inner, 1 Boundary
    inner_indices = np.where((~is_padding) & (dist_to_axis < min(length, width) * 0.3))[0]
    local_candidates = []
    if len(inner_indices) > 0:
        idx = np.random.choice(inner_indices)
        local_candidates.append((rank, idx, "Inner"))
    
    dist_to_boundary_x = np.minimum(np.abs(centers[:, 0] - 0.0), np.abs(length - centers[:, 0]))
    dist_to_boundary_z = np.minimum(np.abs(centers[:, 2] - 0.0), np.abs(width - centers[:, 2]))
    min_dist_to_boundary = np.minimum(dist_to_boundary_x, dist_to_boundary_z)
    
    wall_indices = np.where((~is_padding) & (min_dist_to_boundary <= 0.5 * dx_m + 1e-6))[0]
    if len(wall_indices) > 0:
        idx = np.random.choice(wall_indices)
        local_candidates.append((rank, idx, "Boundary"))
    ...
    if rank == 0:
        ...
        # Pick up to 6 unique points
        if len(flattened) > 6:
            import random
            test_points = random.sample(flattened, 6)
        else:
            test_points = flattened
    ...
    for point in test_points:
        ...
        all_results.append({
            'label': label,
            'a_dC': a_dC, 'a_dV': a_dV,
            'errors_C': point_errors_C, 'errors_V': point_errors_V,
            'fd_C': point_fd_C, 'fd_V': point_fd_V
        })
    ...
    if rank == 0:
        try:
            ...
            for res in all_results:
                label = f"{res['label']} (a_dC={res['a_dC']:.1e})"
                axs[0,0].loglog(dh_values, res['errors_C'], marker='o', label=label)
                axs[0,1].loglog(dh_values, res['errors_V'], marker='o', label=label)
                axs[1,0].semilogx(dh_values, res['fd_C'], marker='x', label=f"FD {res['label']}")
                axs[1,0].axhline(y=res['a_dC'], color='gray', linestyle='--', alpha=0.3)
                axs[1,1].semilogx(dh_values, res['fd_V'], marker='x', label=f"FD {res['label']}")
                axs[1,1].axhline(y=res['a_dV'], color='gray', linestyle='--', alpha=0.3)

            axs[0,0].set_title('Sensitivity Error: dC/drho'); axs[0,0].set_ylabel('Rel Error %')
            axs[0,1].set_title('Sensitivity Error: dV/drho'); axs[0,1].set_ylabel('Rel Error %')
            axs[1,0].set_title('FD dC/drho vs dh'); axs[1,0].set_ylabel('dC/drho')
            axs[1,1].set_title('FD dV/drho vs dh'); axs[1,1].set_ylabel('dV/drho')
            
            for ax in axs.flat:
                ax.set_xlabel('Step size (dh)'); ax.legend(fontsize='small'); ax.grid(True, which="both", ls="--", alpha=0.5); ax.invert_xaxis()

            plt.tight_layout(); os.makedirs('results', exist_ok=True)
            plt.savefig('results/fd_check_parallel.png', dpi=300)
        except (ImportError, PermissionError): pass
